        ...
        for x in range(len(grid)):
            for y in range(len(grid[0])):
                for orientation in range(4):
                    # if at the goal do this
                    if goal[0] == x and goal[1] == y:
                        if value[orientation][x][y] > 0:
                            change = True
                            policy[orientation][x][y] = '*'
                            value[orientation][x][y] = 0
                    # if grid is zero  do this
                    elif grid[x][y] == 0:
                         #calculate the three ways to propogate
                        for i in range(3):
...
expand = optimum_policy2D(grid, init, goal, cost)
for e in expand:
    print(e)
